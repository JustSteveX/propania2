...
export default class LoginScene extends Phaser.Scene {
	...
	create() {
		...
		this.emailInput.addEventListener('input', (e) => {
			...
			if (emailRegex.test(email)) {
				...
				this.emailInput.style.border = email
					? '2px solid #f44336'
					: '1px solid #ccc';
			}
		});
		...
		this.passwordtext.id = 'passwordtext';
	...
	private resizeUI() {
		...
		const bg = this.children.getByName(
			'background'
		...
		const propania = this.children.getByName(
			'propania2'
		) as Phaser.GameObjects.Image;
		if (propania) {
			propania.setPosition(centerX, centerY - 250);
		}

		const loginBtn = this.children.getByName(
			'loginbutton'
		) as Phaser.GameObjects.Image;
		if (loginBtn) {
			loginBtn.setPosition(centerX, centerY + 50);
		}

		const registerBtn = this.children.getByName(
			'registerbutton'
		) as Phaser.GameObjects.Image;
		if (registerBtn) {
			registerBtn.setPosition(centerX, centerY + 120);
		}
